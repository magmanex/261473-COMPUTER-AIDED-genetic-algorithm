...
GA_ELITSIZE = int(GA_POPSIZE * GA_ELITERATE) #Number of elites 
GA_MUTCHANCE = 4
...
def mate_pop(pop , buf):
    ...
    while i < GA_POPSIZE :
        randint = int((random.random()*100)%TARGET_LEN)
        j = 0
...
def mutate(pop):
    for i in range(GA_ELITSIZE):
        rand_mem = random.randint(0,GA_MUTCHANCE - 1)
        rand_cms = random.randint(0,CHROMOSOME_MAX - 1)
        pop[0].sol[rand_mem] = rand_cms
...
def main():
    ...
    pop_a = [Ga_memb() for i in range(GA_POPSIZE)]
    pop_b = [Ga_memb() for i in range(GA_POPSIZE)]
